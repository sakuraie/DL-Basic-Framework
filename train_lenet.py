...
split_dir = os.path.join(".", "data", "animal_split")
...
# ============================ test ============================
test_loader = DataLoader(dataset=test_data, batch_size=2500)
correct_test = 0.
total_test = 0.
loss_test = 0.
net.eval()
for j, data in enumerate(test_loader):
    ...
    print("Test:\t Iteration[{:0>3}/{:0>3}] Acc:{:.2%}".format(
        j + 1, len(test_loader), correct_test / total_test))
...
#     _, predicted = torch.max(outputs.data, 1)
#
#     animal = 'cat' if predicted.numpy()[0] == 0 else 'dog'
#     print("模型识别结果：{}".format(animal))
